...
export const cadastrarEstoque = async (req, res) => {
	try {
		...
		if (!nome_produto || quantidade === undefined) {
			return res.status(400).json({ erro: "nome_produto e quantidade são obrigatórios" });
		}
		...
		const estoque = await db.query(
			`SELECT * FROM estoque WHERE id_produto=$1 AND id_usuario=$2`,
			[id_produto, req.user.id]
		);

		if (estoque.rows.length > 0) {
			return res.status(400).json({ erro: "Estoque já existe para esse produto" });
		}

		// inserir estoque com dono
		await db.query(
			`INSERT INTO estoque (id_produto, quantidade, id_usuario)
			 VALUES ($1,$2,$3)`,
			[id_produto, quantidade, req.user.id]
		);

		res.json({ msg: "Estoque cadastrado com sucesso" });
