...
export const criar = async (req, res) => {
  try {
    ...
    const catResult = await db.query("SELECT id FROM categorias WHERE id=$1", [idCategoria]);
    ...
    await db.query(`INSERT INTO estoque (id_produto, quantidade) VALUES ($1,$2)`, [
      id_produto,
      quantidade || 0,
    ]);
...
export const atualizar = async (req, res) => {
  try {
    ...
    const catResult = await db.query("SELECT id FROM categorias WHERE id=$1", [categoria]);
    if (catResult.rows.length === 0) {
      return res.status(400).json({ erro: "Categoria não encontrada" });
    }
    ...
    if (quantidade !== undefined) {
      await db.query(`UPDATE estoque SET quantidade=$1 WHERE id_produto=$2`, [quantidade, id]);
    }
